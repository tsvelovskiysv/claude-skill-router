...
— если не совпал, скилл изменён с момента каталогизации/аудита → пропуск (нужен --force);
(3) имя скилла и пути пакета валидируются от path-traversal. Тела нигде не тиражируются —
берутся из первоисточника, поэтому вредонос не распространяется через этот инструмент.
"""
import os
import io
import re
...
def fetch_body(canon, canon_path, token=None):
    """SKILL.md из репо-источника (default branch) → bytes или None."""
...
def install(project, selection, base_names=None, token=None, allow_changed=False, log=print):
    ...
    installed, blocked, changed, invalid, flagged, failed = [], [], [], [], [], []
    for it in selection:
        ...
        body = fetch_body(it.get("canon"), it.get("canon_path"), token)
        if not body:
